  ...
  for (var i = 0; i < length; ++i) {
    ...
    if (options != null) {
      for (name in options) {
        ...
        if (target !== copy) {
          if (deep && copy && ((copyIsArray = lang_js_1.isArray(copy)) || lang_js_1.isPlainObject(copy))) {
            if (copyIsArray) {
              copyIsArray = false;
              clone = src && lang_js_1.isArray(src) ? src : [];
            } else {
              clone = src && lang_js_1.isPlainObject(src) ? src : {};
            }
