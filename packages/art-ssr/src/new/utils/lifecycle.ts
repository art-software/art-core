 ...
 *
 */
export const runLifecycle = (lifecycle: string, plugins: any[], manager: BatchManager, token?: string) => {
  try {
    const promise = Promise.all(
      plugins
        .filter(hasMethod(lifecycle))
 ...
 */
export const runLifecycleSync = (lifecycle: string, plugins: any[], manager: BatchManager, token?: string) => {
  plugins
    .filter(hasMethod(lifecycle))
    .forEach((plugin) => {
      return plugin[lifecycle](manager.contextFor(plugin, token));
 ...
 */
export const errorSync = (err: any, plugins: any[], manager: BatchManager, token?: string) => {
  plugins
