...
const setSwConfigInArtConfigFile = () => {
    ...
    fs_extra_1.default.readFile(artConfigFilePath)
        ...
        if (isExistServiceWorkerConfig) {
            console.log(chalk_1.default.gray(`  Service worker config has existed in art.config.js, not set again.`));
            return;
        }
        // 修改 dll.vendors
        ...
        fs_extra_1.default.writeFile(artConfigFilePath, targetCode).then(() => {
            console.log(chalk_1.default.green(`  Set service worker config in ${chalk_1.default.cyanBright('art.config.js')} file finished!`));
        });
    });
};
