...
import { getUserToken, getAppId, getUdid, getChannel } from './requestConfig';
import qs from 'qs';
...
export default class WebApi {
  ...
  public request(method: string, url: string, config: RequestConfig = {}) {
    const inputRawData = this.adjustParameter(method, url, config);
  ...
  public adjustParameter(method: string, url: string = '', config: RequestConfig = {}) {
    let dto = (result) => {
      return result;
    };
    let data: any = {};
    ...
    if (method === 'POST') {
      merge(true, this.apiConfig.headers, { 'Content-Type': 'application/x-www-form-urlencoded' });
