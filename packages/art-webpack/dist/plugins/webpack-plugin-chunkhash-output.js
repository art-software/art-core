...
class OutputHash {
    constructor({ validateOutput = false, validateOutputRegex = /bundle.*\.(js|css)/, hashDigestLength = 0, version = '', enableBundleHashName = false } = {}) {
    ...
    apply(compiler) {
        ...
        compiler.hooks.compilation.tap('OutputHash', (compilation) => {
            ...
            hashFn = (input) => {
                const hashObj = crypto_1.default.createHash(hashFunction).update(input);
                if (hashSalt) {
                ...
                const extractInOrder = (group) => {
                    ...
                    group.chunks.forEach((chunk) => {
                        Array.from(chunk.groupsIterable).forEach((parentGroup) => {
                            if (!visitedGroups.includes(parentGroup)) {
                                extractInOrder(parentGroup);
                            }
                        });
                        if (!sortedChunks.includes(chunk)) {
    ...
    concatContentInAsset(asset, resultContent) {
        if (!asset) {
